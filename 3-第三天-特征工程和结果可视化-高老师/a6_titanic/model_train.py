...
# (1) 读取数据集
data_train = pd.read_csv("a6_titanic/data/train.csv")
print(data_train)
# (2) 特征工程 - 处理缺失值
...
print("看每列性质，空值和类型", df)

print("看每列统计信息", df.describe())

# (4) 特征工程 - 特征抽取
# 我们把需要的feature字段取出来，转成numpy格式，使用scikit-learn中的LogisticRegression建模
train_df = df.filter(regex='Survived|Age_.*|SibSp|Parch|Fare_.*|Cabin_.*|Embarked_.*|Sex_.*|Pclass_.*')
#train_df.to_csv("processed_titanic.csv" , encoding = "utf-8")
#train_np = train_df.as_matrix() #替换为下列属性，不使用该方法
...
#clf = AdaBoostClassifier(tree.DecisionTreeClassifier(max_depth=4), n_estimators=300)

clf = linear_model.LogisticRegression(C=100.0, penalty='l1', solver='liblinear',tol=1e-6)
# max_depth=10, n_estimators=10
# max_depth=50, n_estimators=10
# max_depth=10, n_estimators=50
# max_depth=50, n_estimators=50

#clf = RandomForestClassifier(criterion='gini', max_depth=5, n_estimators=10)
# fit / predict
